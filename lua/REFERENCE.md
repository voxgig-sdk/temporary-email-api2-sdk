# TemporaryEmailApi2 Lua SDK Reference

Complete API reference for the TemporaryEmailApi2 Lua SDK.
...
```lua
local result, err = client:EmailGeneration(nil):load({ id = "email_generation_id" }, nil)
```
...
```lua
local result, err = client:EmailInbox(nil):load({ id = "email_inbox_id" }, nil)
```
