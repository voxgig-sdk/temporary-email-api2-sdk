...
The Lua SDK for the TemporaryEmailApi2 API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:EmailGeneration(nil):load({ id = "example_id" }, nil)
...
local result, err = client:TemporaryEmailApi2(nil):load(
  { id = "test01" }, nil
)
