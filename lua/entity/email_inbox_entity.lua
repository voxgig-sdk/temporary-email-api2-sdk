...
local EmailInboxEntity = {}
EmailInboxEntity.__index = EmailInboxEntity


function EmailInboxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmailInboxEntity)
  self._name = "email_inbox"
  self._client = client
...
function EmailInboxEntity:get_name()
  return self._name
end


function EmailInboxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmailInboxEntity.new(self._client, opts)
end
...
function EmailInboxEntity:mark_deleted()
  self._deleted = true
end


function EmailInboxEntity:deleted()
  return true == self._deleted
end


function EmailInboxEntity:data_set(args)
  if args ~= nil then
...
function EmailInboxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmailInboxEntity:match_set(args)
  if args ~= nil then
...
function EmailInboxEntity:match_get()
...
function EmailInboxEntity:stream(action, args, callopts)
...
---@param reqmatch EmailInboxLoadMatch
---@param ctrl? table
---@return EmailInbox
---@return string? err
function EmailInboxEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EmailInboxEntity:_run_op(ctx, post_done)
  local utility = self._utility
