# temporary-email-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TemporaryEmailApi2 SDK as
two agent tools — `temporary-email-api2_list` and `temporary-email-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temporary-email-api2-mcp)
make build

# 2. Provide credentials via the environment
export TEMPORARY_EMAIL_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user temporary-email-api2 \
  -- /absolute/path/to/temporary-email-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./temporary-email-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// temporary-email-api2_list: first page of records
{ "entity": "email_generation" }
{ "entity": "email_generation", "query": { } }

// temporary-email-api2_load: one record by id
{ "entity": "email_generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temporary-email-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user temporary-email-api2 \
     -- "$PWD"/dist/*/temporary-email-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `temporary-email-api2_list` and `temporary-email-api2_load` tools now appear
   in new sessions. Ask the agent to *"list email_generation using temporary-email-api2"*
   and it calls `temporary-email-api2_list` with `{"entity":"email_generation"}`.
...
```sh
export TEMPORARY_EMAIL_API2_APIKEY=sk_live_xxx            # API key
export TEMPORARY_EMAIL_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `temporary-email-api2_list` tool
...
### Call the `temporary-email-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `temporary-email-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `temporary-email-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMPORARY_EMAIL_API2_APIKEY` | API key sent with every request. |
| `TEMPORARY_EMAIL_API2_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

email_generation | email_inbox

### Smoke test via HTTP (raw JSON-RPC)

```sh
./temporary-email-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"temporary-email-api2_load","arguments":{"entity":"email_generation","query":{"id":1}}}}'
```
