...
require_relative 'TemporaryEmailApi2_types'
...
class TemporaryEmailApi2SDK
  ...
  def initialize(options = {})
    ...
    utility = TemporaryEmailApi2Utility.new
    @_utility = utility
    ...
    feature_opts = TemporaryEmailApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TemporaryEmailApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TemporaryEmailApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TemporaryEmailApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TemporaryEmailApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TemporaryEmailApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TemporaryEmailApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TemporaryEmailApi2Error.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TemporaryEmailApi2Error => err
    ...
    if fetched.is_a?(Hash)
      status = TemporaryEmailApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TemporaryEmailApi2Error.new(
        "graphql_error", "TemporaryEmailApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TemporaryEmailApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
