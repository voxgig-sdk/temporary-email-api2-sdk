...
The Ruby SDK for the TemporaryEmailApi2 API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.EmailGeneration(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.TemporaryEmailApi2(nil).load(
  { "id" => "test01" }, nil
)
