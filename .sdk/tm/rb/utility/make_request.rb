...
module TemporaryEmailApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TemporaryEmailApi2Response.new({})
    ...
    if fetch_err
      ...
      response = TemporaryEmailApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TemporaryEmailApi2Response.new(fetched)
    else
