...
// TemporaryEmailApi2 SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TemporaryEmailApi2Omni::makeStructRunner(
    ...
    {
        return TemporaryEmailApi2Omni::objmodel(TemporaryEmailApi2Omni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return TemporaryEmailApi2Omni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return TemporaryEmailApi2Omni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TemporaryEmailApi2Omni::structfix($now, Struct::undef()))
                        !== json_encode(TemporaryEmailApi2Omni::structfix($val, Struct::undef()))) {
