...
// TemporaryEmailApi2 SDK feature test
//
...
class FtCtrl extends TemporaryEmailApi2Control
{
...
class FtClient
{
    ...
    private TemporaryEmailApi2Utility $utility;

    public function __construct(TemporaryEmailApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): TemporaryEmailApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TemporaryEmailApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TemporaryEmailApi2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TemporaryEmailApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TemporaryEmailApi2Config::make_feature($name);
    ...
    private static function buildUrl(TemporaryEmailApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TemporaryEmailApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TemporaryEmailApi2Response($fetched);
        }
    ...
    private function fail(TemporaryEmailApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TemporaryEmailApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
