# temporary-email-api2-cli

boru-driven command-line client **and** interactive REPL for the TemporaryEmailApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temporary-email-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./temporary-email-api2-cli --help

# 3. Provide credentials once, via the environment
export TEMPORARY_EMAIL_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./temporary-email-api2-cli load 1 email_generation            # {id:1} shorthand
./temporary-email-api2-cli load '{id:1}' email_generation       # explicit match map

# 5. Override the API base URL for a single call
TEMPORARY_EMAIL_API2_BASE=https://api.example.com ./temporary-email-api2-cli load 1 email_generation

# 6. No arguments -> interactive REPL
./temporary-email-api2-cli
temporary-email-api2> load 1 email_generation
temporary-email-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temporary-email-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/temporary-email-api2-cli load 1 email_generation
   ```
...
```sh
./temporary-email-api2-cli load 1 email_generation          # scalar shorthand for {id:1}
./temporary-email-api2-cli load '{id:1}' email_generation     # explicit match map
```
...
./temporary-email-api2-cli load 1 email_generation
```
...
Run with no arguments to open a REPL (prompt `temporary-email-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./temporary-email-api2-cli
temporary-email-api2> load 1 email_generation
temporary-email-api2> /help
temporary-email-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `email_generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/temporary-email-api2-cli`. |
...
(`./temporary-email-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
