...
//
// NewTemporaryEmailApi2SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TemporaryEmailApi2SDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewTemporaryEmailApi2SDK(opts)
}
...
func fcCandidates(client *sdk.TemporaryEmailApi2SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TemporaryEmailApi2SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TemporaryEmailApi2SDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.TemporaryEmailApi2Error
