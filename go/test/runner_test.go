...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TEMPORARYEMAILAPI2_TEST_LIVE") == "TRUE" ||
		os.Getenv("TEMPORARYEMAILAPI2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TEMPORARYEMAILAPI2_TEST_EXPLAIN"); explain != "" {
