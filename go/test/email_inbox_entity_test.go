...
func TestEmailInboxEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TEMPORARY_EMAIL_API2_TEST_EMAIL_INBOX_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		emailInboxRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.email_inbox")))
...
func email_inboxBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TEMPORARY_EMAIL_API2_TEST_EMAIL_INBOX_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TEMPORARY_EMAIL_API2_TEST_EMAIL_INBOX_ENTID"])
