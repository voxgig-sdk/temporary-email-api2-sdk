...
import (
	...
	omni "github.com/voxgig-sdk/temporary-email-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TemporaryEmailApi2Error); is {
				out := map[string]any{
					"name":    "TemporaryEmailApi2Error",
...
func omniCtx(arg any, client *sdk.TemporaryEmailApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TemporaryEmailApi2SDK); is {
			client = live
