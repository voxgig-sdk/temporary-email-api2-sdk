...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*TemporaryEmailApi2SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *TemporaryEmailApi2Error {
	return NewTemporaryEmailApi2Error(code, msg, ctx)
}
