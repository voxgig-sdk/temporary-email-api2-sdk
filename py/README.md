...
The Python SDK for the TemporaryEmailApi2 API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.EmailGeneration(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.TemporaryEmailApi2(None).load(
    {"id": "test01"}, None
)
