...
class TestEmailInboxEntity:

    def test_should_create_instance(self):
        testsdk = TemporaryEmailApi2SDK.test(None, None)
        ent = testsdk.EmailInbox(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _email_inbox_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "email_inbox." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TEMPORARY_EMAIL_API2_TEST_EMAIL_INBOX_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        email_inbox_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.email_inbox")))
        email_inbox_ref01_data = None
        if len(email_inbox_ref01_data_raw) > 0:
...
def _email_inbox_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/email_inbox/EmailInboxTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "TEMPORARY_EMAIL_API2_TEST_EMAIL_INBOX_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TEMPORARY_EMAIL_API2_TEST_EMAIL_INBOX_ENTID"))
