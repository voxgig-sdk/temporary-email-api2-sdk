# TemporaryEmailApi2 SDK test runner
...
class TemporaryEmailApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TemporaryEmailApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TemporaryEmailApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TemporaryEmailApi2TestRunner.getenv("TEMPORARY_EMAIL_API2_TEST_LIVE")
        override = TemporaryEmailApi2TestRunner.getenv("TEMPORARY_EMAIL_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TemporaryEmailApi2TestRunner.getenv(key)
        ...
        explain = TemporaryEmailApi2TestRunner.getenv("TEMPORARY_EMAIL_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TEMPORARY_EMAIL_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TemporaryEmailApi2TestRunner._test_control is not None:
            return TemporaryEmailApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TemporaryEmailApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TemporaryEmailApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TemporaryEmailApi2TestRunner.load_test_control()
...
def load_env_local():
    TemporaryEmailApi2TestRunner.load_env_local()


def env_override(m):
    return TemporaryEmailApi2TestRunner.env_override(m)


def entity_data(v):
    return TemporaryEmailApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TemporaryEmailApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TemporaryEmailApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TemporaryEmailApi2TestRunner.load_test_control()


def live_client_options():
    return TemporaryEmailApi2TestRunner.live_client_options()


def live_delay_ms():
    return TemporaryEmailApi2TestRunner.live_delay_ms()
