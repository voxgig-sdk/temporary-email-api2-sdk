...
from temporaryemailapi2_sdk.temporaryemailapi2_types import (
    EmailGeneration,
    EmailGenerationLoadMatch,
)
...
class EmailGenerationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EmailGenerationEntity(self._client, opts)
    ...
    def data_get(self) -> EmailGeneration:
    ...
    def match_get(self) -> EmailGeneration:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EmailGeneration:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EmailGeneration().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
