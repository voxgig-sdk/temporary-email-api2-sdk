...
class TemporaryEmailApi2MakeRequest
{
    public static function call(TemporaryEmailApi2Context $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new TemporaryEmailApi2Response(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new TemporaryEmailApi2Response($fetched);
        } else {
