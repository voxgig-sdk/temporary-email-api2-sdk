...
//    only - a typed TemporaryEmailApi2Context there would make every ctx
...
final class TemporaryEmailApi2Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TemporaryEmailApi2Context($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TemporaryEmailApi2Spec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TemporaryEmailApi2Result($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TemporaryEmailApi2Error('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TemporaryEmailApi2Response($clean['response']);
    ...
    public static function err_from_map($m): ?TemporaryEmailApi2Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TemporaryEmailApi2Error(is_string($code) ? $code : '', $msg);
    }
