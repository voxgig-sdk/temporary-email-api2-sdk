...
// TemporaryEmailApi2 SDK retry feature
...
class TemporaryEmailApi2RetryFeature extends TemporaryEmailApi2BaseFeature
{
    ...
    public function init(TemporaryEmailApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TemporaryEmailApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TemporaryEmailApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TemporaryEmailApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
