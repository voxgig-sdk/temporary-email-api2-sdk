...
class TemporaryEmailApi2SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TemporaryEmailApi2Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TemporaryEmailApi2Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TemporaryEmailApi2Features::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TemporaryEmailApi2Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TemporaryEmailApi2Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TemporaryEmailApi2Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TemporaryEmailApi2Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TemporaryEmailApi2Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TemporaryEmailApi2Error($op . "_allow",
                "TemporaryEmailApi2SDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TemporaryEmailApi2Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TemporaryEmailApi2Error("graphql_error",
                "TemporaryEmailApi2SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function EmailGeneration($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_email_generation === null) {
                $this->_email_generation = new EmailGenerationEntity($this, null);
            }
            return $this->_email_generation;
        }
        return new EmailGenerationEntity($this, $data);
    }
    ...
    public function EmailInbox($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_email_inbox === null) {
                $this->_email_inbox = new EmailInboxEntity($this, null);
            }
            return $this->_email_inbox;
        }
        return new EmailInboxEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new TemporaryEmailApi2SDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
