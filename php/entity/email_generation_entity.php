...
// TemporaryEmailApi2 SDK EmailGeneration entity
...
class EmailGenerationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EmailGenerationEntity($this->_client, $opts);
    }

    /**
     * @param EmailGeneration|array $args EmailGeneration data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TemporaryEmailApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EmailGeneration|array The current EmailGeneration data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EmailGeneration fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TemporaryEmailApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EmailGeneration fields).
     */
    ...
    /**
     * Load a single EmailGeneration.
     *
     * @param EmailGenerationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EmailGenerationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EmailGeneration|array The loaded EmailGeneration as an assoc-array at the
     *   SDK boundary; throws TemporaryEmailApi2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TemporaryEmailApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
