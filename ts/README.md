# TemporaryEmailApi2 TypeScript SDK



The TypeScript SDK for the TemporaryEmailApi2 API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.EmailGeneration()` — each with a small set of operations (`load`)
...
```ts
new TemporaryEmailApi2SDK(options?: {
  base?: string
  prefix?: string
  suffix?: string
...
| Method | Returns | Description |
| --- | --- | --- |
| `TemporaryEmailApi2SDK.test(testopts?, sdkopts?)` | `TemporaryEmailApi2SDK` | Create a test-mode client. |
...
```ts
const email_generation = await client.EmailGeneration().load()
```
...
├── src/
│   ├── TemporaryEmailApi2SDK.ts        # Main SDK class
