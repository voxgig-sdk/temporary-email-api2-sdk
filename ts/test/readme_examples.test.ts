...
//     (\`new TemporaryEmailApi2SDK(...)\` / \`TemporaryEmailApi2SDK.test(...)\` become
//     \`TemporaryEmailApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TemporaryEmailApi2SDK'
...
const TEST_SEED = {"entity":{"email_generation":{"test01":{"id":"test01"}},"email_inbox":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TemporaryEmailApi2SDK(...)\` and any
// \`TemporaryEmailApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TemporaryEmailApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TemporaryEmailApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
